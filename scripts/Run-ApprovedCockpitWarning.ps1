...
$applyLogPath = Join-Path $logDir "ApprovedCockpitWarning.log"
$captureLogPath = Join-Path $logDir "ApprovedCockpitWarningComparison.log"
...
& (Join-Path $PSScriptRoot "Invoke-UnityEditorBridge.ps1") -Command "EnsureApprovedCockpitWarning" -LogPath $applyLogPath -TimeoutSeconds 240
if ($LASTEXITCODE -ne 0) {
  exit $LASTEXITCODE
}

& (Join-Path $PSScriptRoot "Invoke-UnityEditorBridge.ps1") -Command "CaptureApprovedCockpitWarningComparison" -LogPath $captureLogPath -TimeoutSeconds 240
if ($LASTEXITCODE -ne 0) {
  exit $LASTEXITCODE
}
...
if ($applyLog -notmatch "Approved cockpit 04 warning applied\.") {
  Write-Error "Approved cockpit warning apply success marker was not found. See $applyLogPath"
  exit 1
}
...
if ($captureLog -notmatch "Approved cockpit 04 warning Unity comparison snapshots saved:") {
  Write-Error "Approved cockpit warning comparison capture marker was not found. See $captureLogPath"
  exit 1
}
...
if ($applyLog -match "Scripts have compiler errors|error CS\d+|Unity editor bridge failed" -or
  ...
  Write-Error "Approved cockpit warning logs contain errors. See $applyLogPath and $captureLogPath"
  exit 1
}
