...
$requestPath = Join-Path $projectRoot "Logs\Phase18MvpPlaytestLoopSmoke.request"
$activePath = Join-Path $projectRoot "Logs\Phase18MvpPlaytestLoopSmoke.active"
$logPath = Join-Path $projectRoot "Logs\Phase18MvpPlaytestLoopSmoke.log"
$timeoutSeconds = 90
...
if (Test-Path -LiteralPath $activePath) {
  ...
  if ($activeItem.LastWriteTime -lt (Get-Date).AddMinutes(-2)) {
    ...
    Write-Error "A Phase 18 MVP playtest loop smoke request is already active. See $activePath"
    exit 1
  }
}

$requestId = [guid]::NewGuid().ToString("N")
Set-Content -LiteralPath $logPath -Value "Phase 18 MVP playtest loop smoke pending: $requestId" -Encoding UTF8
@(
  "id=$requestId",
  "logPath=$logPath"
...
while ((Get-Date) -lt $deadline) {
  if (Test-Path -LiteralPath $logPath) {
    ...
    if ($log -match "Phase 18 MVP playtest loop smoke completed: $requestId") {
      if ($log -match "Result: Passed") {
        exit 0
      }

      Write-Error "Phase 18 MVP playtest loop smoke failed. See $logPath"
      exit 1
    }
  }
...
Write-Error "Phase 18 MVP playtest loop smoke timed out after $timeoutSeconds seconds. See $logPath"
exit 1
