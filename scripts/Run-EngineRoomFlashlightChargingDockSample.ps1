...
$logPath = Join-Path $logDir "EngineRoomFlashlightChargingDockSample.log"
$stdoutPath = Join-Path $logDir "EngineRoomFlashlightChargingDockSample.stdout.log"
$stderrPath = Join-Path $logDir "EngineRoomFlashlightChargingDockSample.stderr.log"
$scriptPath = Join-Path $projectRoot "scripts\GenerateEngineRoomFlashlightChargingDockSample.py"
$sampleRoot = Join-Path $projectRoot "artSample\engine_room_flashlight_charging_dock"
...
foreach ($relative in $requiredFiles) {
  ...
  if (-not (Test-Path -LiteralPath $path)) {
    Write-Error "Missing generated engine room flashlight charging dock sample file: $path"
    exit 1
  }
}
