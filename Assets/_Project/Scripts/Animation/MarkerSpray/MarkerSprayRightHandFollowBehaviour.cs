...
namespace Bellerophon.PlayerAnimation
{
    ...
    public sealed class MarkerSprayRightHandFollowBehaviour : MonoBehaviour
    {
        ...
        [SerializeField] private MarkerSprayIdleCarryProfile profile;
        ...
        public MarkerSprayIdleCarryProfile Profile => profile;
        public GameObject SprayHolder => sprayHolder;
        public Transform SprayModel => sprayModel;

        public void Configure(MarkerSprayIdleCarryProfile carryProfile)
        {
        ...
        {
            ResolveHand();
            Quaternion handRotation = rightHand.rotation * profile.RotationOffsetFromHand;
        ...
        {
            foreach (MarkerSprayBoneRotation carryPose in profile.RightArmPose)
            {
                Transform bone = transform.Find(carryPose.Path);
                if (bone == null)
                    throw new MissingReferenceException(
                        "MarkerSpray right-arm pose path could not be resolved: " +
                        carryPose.Path);
                if (!TryFindSourcePose(carryPose.Path, out Quaternion sourceRotation))
                    throw new MissingReferenceException(
                        "MarkerSpray source right-arm pose path could not be resolved: " +
                        carryPose.Path);
        ...
        {
            foreach (MarkerSprayBoneRotation sourcePose in profile.SourceRightArmPose)
            {
        ...
        private void ConstrainPalmToPlayerLeft()
        {
            ...
            for (int iteration = 0; iteration < 8; iteration++)
            {
                ...
                Vector3 palmNormal = Vector3.Cross(palmWidth, fingerDirection).normalized;
                if (Vector3.Angle(palmNormal, playerLeft) <=
                    PalmLeftMaximumDeviationDegrees) return;
                hand.rotation = Quaternion.FromToRotation(palmNormal, playerLeft) * hand.rotation;
            }
        }
        ...
        private Transform FindDescendant(string boneName)
        {
            ...
            throw new MissingReferenceException(
                "MarkerSpray right-arm bone could not be resolved: " + boneName);
        }
        ...
        private void ApplyRightArmPose()
        {
            foreach (MarkerSprayBoneRotation pose in profile.RightArmPose)
            {
                ...
                if (bone == null)
                    throw new MissingReferenceException(
                        "MarkerSpray right-arm pose path could not be resolved: " + pose.Path);
        ...
        {
            if (profile == null)
                throw new MissingReferenceException("MarkerSpray carry profile is missing.");
            if (rightHand == null) rightHand = transform.Find(profile.RightHandPath);
            if (rightHand == null)
                throw new MissingReferenceException(
                    "MarkerSpray right-hand path could not be resolved: " +
        ...
        private void EnsureSprayInstance()
        {
            if (profile == null || profile.SprayPrefab == null)
                throw new MissingReferenceException("MarkerSpray model prefab is missing.");
            if (sprayHolder != null && instantiatedPrefab == profile.SprayPrefab)
            {
                if (sprayModel == null)
                    sprayModel = sprayHolder.transform.Find("MarkingSpray_Model");
                return;
            }
            DestroySprayInstance();
            sprayHolder = new GameObject("MarkerSpray_Prop");
