...
{
    public sealed class ElectricMineIdleLocomotionCycleBehaviour : StateMachineBehaviour
    {
        ...
        public void ConfigureGrip(string[] paths, Quaternion[] rotations)
        {
            if (paths == null || rotations == null || paths.Length != rotations.Length ||
                paths.Length == 0)
                throw new ArgumentException("Electric mine grip data is incomplete.");
            ...
            int layerIndex)
        {
            ...
            resolvedBones.Remove(animator.GetInstanceID());
        }
        ...
        private Transform[] ResolveBones(Animator animator)
        {
            if (gripBonePaths == null || gripLocalRotations == null ||
                ...
                throw new InvalidOperationException(
                    "Electric mine grip pose was not configured.");
            ...
            for (int index = 0; index < gripBonePaths.Length; index++)
            {
                bones[index] = animator.transform.Find(gripBonePaths[index]);
                if (bones[index] == null)
                    throw new MissingReferenceException(
                        "Electric mine grip bone is missing: " + gripBonePaths[index]);
            }
