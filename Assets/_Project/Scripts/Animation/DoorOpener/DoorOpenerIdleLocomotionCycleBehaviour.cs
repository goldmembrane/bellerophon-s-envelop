...
{
    public sealed class DoorOpenerIdleLocomotionCycleBehaviour : StateMachineBehaviour
    {
        public const int MotionCount = 6;
        public const float SecondsPerMotion = 1f;
        public const float TotalCycleDurationSeconds = MotionCount * SecondsPerMotion;
        ...
        private static readonly HashSet<int> ValidationOverrides =
            new HashSet<int>();

        public static void SetValidationOverride(Animator animator, bool enabled)
        {
            if (animator == null) return;
            int id = animator.GetInstanceID();
            if (enabled) ValidationOverrides.Add(id);
            else ValidationOverrides.Remove(id);
        }
        ...
        public override void OnStateEnter(
            ...
            int layerIndex)
        {
            if (ValidationOverrides.Contains(animator.GetInstanceID())) return;
            ...
            int layerIndex)
        {
            ...
            ValidationOverrides.Remove(animator.GetInstanceID());
        }
