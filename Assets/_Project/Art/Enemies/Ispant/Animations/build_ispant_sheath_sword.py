...
RIGID_MUSKET_NAME = "Ispant_Sheath_RigidMusket"
...
EXPECTED_ACTION_FRAMES = (1.0, 100.0)
...
def compare_static_geometry(source_geometry, static_source):
    ...
    if maximum_error > GEOMETRY_TOLERANCE:
        raise RuntimeError(
            f"The supplied FBX geometry differs from the static Ispant: {maximum_error}"
        )
...
def split_rigid_musket(body, armature):
    if triangle_count(body.data) != EXPECTED_SOURCE_BODY_TRIANGLES:
        raise RuntimeError("The supplied Ispant body topology differs.")
    ...
    if triangle_count(body.data) != EXPECTED_ANIMATED_BODY_TRIANGLES:
        raise RuntimeError("The animated Ispant body topology differs after musket separation.")
    if triangle_count(musket.data) != EXPECTED_MUSKET_TRIANGLES:
        raise RuntimeError("The rigid Ispant musket topology differs.")
...
def configure_scene(source):
    ...
    if source_sword is None or source_sword.type != "MESH":
        raise RuntimeError("The supplied incorrectly weighted sword reference is missing.")
...
def main():
    ...
    print(
        ...
        " SourceSwordRemoved=True StaticSwordAddedInUnity=True"
    )
