...
{
    [InitializeOnLoad]
    internal static class RebellionForwardScanPlayModeCapture
    {
        ...
        private const string SessionStateKey =
            "Bellerophon.RebellionForwardScanCapture.State";
        private const string SessionIndexKey =
            "Bellerophon.RebellionForwardScanCapture.Index";
        private const string SessionStartTimeKey =
            "Bellerophon.RebellionForwardScanCapture.StartTime";
        private const string SessionFailureKey =
            "Bellerophon.RebellionForwardScanCapture.Failure";
        ...
        static RebellionForwardScanPlayModeCapture()
        {
            ...
            Action<Exception> failCallback)
        {
            ...
            if (EditorApplication.isPlayingOrWillChangePlaymode)
            {
                throw new InvalidOperationException(
                    "Cannot start the Rebellion forward scan review while " +
        ...
        private static void BeginRuntimePlayback()
        {
            ...
            animator.Play(
                RebellionForwardScanTool.AnimatorStateName,
                0,
                0f);
            animator.Update(0f);
        ...
        private static void CaptureRuntimeFramesWhenDue()
        {
            ...
            var normalizedTime =
                (float)(CaptureTimes[index] /
                        RebellionForwardScanTool.LoopSeconds);
            animator.Play(
                RebellionForwardScanTool.AnimatorStateName,
                0,
                normalizedTime);
            animator.Update(0f);
            RebellionForwardScanTool.CaptureRuntimeFrame(PanelPath(index));
            index++;
            ...
            RebellionForwardScanTool.ComposeRuntimeReview(
                panels,
                RebellionForwardScanTool.FinalReviewAbsolutePath);
        ...
        private static void FinishSuccess()
        {
            ...
            CleanupSession();
            callback?.Invoke(
                "Rebellion actual Animator forward scan frames captured " +
                "across the complete 1.6-second round trip.");
        }
        ...
        private static void FinishFailure()
        {
            var error = SessionState.GetString(
                SessionFailureKey,
                "Rebellion forward scan Play Mode capture failed.");
            var callback = fail;
            CleanupSession();
        ...
        {
            TryDelete(RebellionForwardScanTool.FinalReviewAbsolutePath);
        ...
        private static string PanelPath(int index)
        {
            return Path.Combine(
                ProjectRoot,
                "Logs",
                "Rebellion_ForwardScan_PlayMode_" + index + ".png");
        }
