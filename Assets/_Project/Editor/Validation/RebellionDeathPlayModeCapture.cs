...
namespace Bellerophon.Editor.RebellionCargoRunScene
{
    ...
    internal static class RebellionDeathPlayModeCapture
    {
        ...
        static RebellionDeathPlayModeCapture()
        {
            ...
            Action<Exception> failCallback)
        {
            ...
            if (EditorApplication.isPlayingOrWillChangePlaymode)
            {
                throw new InvalidOperationException(
                    "Cannot start the Rebellion death review while Unity is " +
                    "entering Play Mode.");
            }
            ...
            {
                throw new InvalidOperationException(
                    "CargoRunMvp must be clean before the Rebellion death " +
                    "review.");
            }
        ...
        private static void Tick()
        {
            ...
            try
            {
                ...
                if (state == Capturing)
                {
                    ...
                    {
                        throw new InvalidOperationException(
                            "Unity left Play Mode before the Rebellion death " +
                            "review finished.");
                    }
        ...
        private static void BeginRuntimePlayback()
        {
            ...
            animator.Play(
                RebellionAttackModeTransitionTool.DeathAnimatorStateName,
                0,
                0f);
            animator.Update(0f);
        ...
        private static void CaptureRuntimeFramesWhenDue()
        {
            ...
            var normalizedTime =
                (float)(CaptureTimes[index] /
                        RebellionAttackModeTransitionTool.DeathLoopSeconds);
            animator.Play(
                RebellionAttackModeTransitionTool.DeathAnimatorStateName,
                0,
                normalizedTime);
            animator.Update(0f);
            RebellionAttackModeTransitionTool.CaptureDeathRuntimeFrame(
                PanelPath(index));
            index++;
        ...
        private static void FinishSuccess()
        {
            ...
            CleanupSession();
            callback?.Invoke(
                "Rebellion actual Animator death frames captured across the " +
                "complete 1.2-second loop.");
        }
        ...
        private static void FinishFailure()
        {
            var error = SessionState.GetString(
                SessionFailureKey,
                "Rebellion death Play Mode capture failed.");
            var callback = fail;
            CleanupSession();
        ...
        private static string PanelPath(int index)
        {
            return Path.Combine(
                ProjectRoot,
                "Logs",
                "Rebellion_Death_PlayMode_" + index + ".png");
        }
