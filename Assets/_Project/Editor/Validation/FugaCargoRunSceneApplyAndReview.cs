...
namespace Bellerophon.Editor.FugaCargoRunScene
{
    internal static class FugaCargoRunSceneApplyAndReview
    {
        ...
        private static readonly PlacementSpec[] PlacementSpecs =
        {
            ...
            new PlacementSpec("Fuga_01_Idle", "Fuga_Idle_SlowWingbeat", 0.42f, true, "Idle: slow continuous wingbeat"),
            new PlacementSpec("Fuga_02_Move", "Fuga_Move_FastWingbeat", 0.24f, true, "Move: faster and larger wingbeat"),
            new PlacementSpec("Fuga_03_Attack", "Fuga_Attack_WingtipStrike", 0.62f, true, "Attack: lean back, lunge, strike with wingtip"),
            new PlacementSpec("Fuga_04_Hit", "Fuga_Hit_SquashRecoil", 0.24f, true, "Hit: recoil, squash recovery, wing droop"),
            new PlacementSpec("Fuga_05_Death", "Fuga_Death_FallAndFold", 1.72f, false, "Death: lose lift, fall, fold wings near floor"),
            new PlacementSpec("Fuga_06_Consume", "Fuga_Consume_BiteForward", 0.52f, true, "Consume: open mouth, lean forward, bite")
        };
        ...
        private static Dictionary<string, AnimationClip> EnsureAnimationClips(GameObject prefab, Vector3 sceneScale)
        {
            ...
            foreach (var spec in PlacementSpecs)
            {
                ...
                SaveClip(clip, UnityAnimationFolder + "/" + spec.ClipName + ".anim", spec.Looping);
                clips[spec.ClipName] = AssetDatabase.LoadAssetAtPath<AnimationClip>(
                    UnityAnimationFolder + "/" + spec.ClipName + ".anim");
            }
            ...
            Vector3 sceneScale)
        {
            ...
            for (var i = 0; i < PlacementSpecs.Length; i++)
            {
                ...
                ConfigureInstancePhysics(instance);
        ...
        private static void ConfigureInstancePhysics(GameObject instance)
        {
            ...
            driver.Configure(rigidbody, target, true);
        }
        ...
        {
            var clip = CreateClip("Fuga_Idle_SlowWingbeat", 1.8f);
            AddBodyHover(clip, paths.Model, 1.8f, MetersToLocal(0.02f, sceneScale.y));
            AddWingBeat(clip, paths, 1.8f, 12f, 5f, 0f);
        ...
        {
            var clip = CreateClip("Fuga_Move_FastWingbeat", 0.9f);
            AddBodyHover(clip, paths.Model, 0.9f, MetersToLocal(0.04f, sceneScale.y));
            AddWingBeat(clip, paths, 0.9f, 24f, 9f, 0.15f);
            AddMotionTargetForwardPulse(clip, paths.MotionTarget, 0.9f, MetersToLocal(0.18f, sceneScale.z));
            return clip;
        }

        private static AnimationClip BuildAttackClip(AnimationPaths paths, Vector3 sceneScale)
        {
            var clip = CreateClip("Fuga_Attack_WingtipStrike", 1.18f);
            SetVectorCurve(clip, paths.Model, "localPosition", 0f, Vector3.zero, 0.28f, MetersToLocal(new Vector3(0f, 0.015f, -0.08f), sceneScale), 0.62f, MetersToLocal(new Vector3(0f, -0.015f, 0.18f), sceneScale), 1.18f, Vector3.zero);
            SetEulerX(clip, paths.Model, 0f, 0f, 0.28f, -8f, 0.62f, 13f, 1.18f, 0f);
            SetWingEuler(clip, paths, 0f, 0f, 0.28f, -15f, 0.62f, 34f, 0.84f, 18f, 1.18f, 0f);
            AddMotionTargetForwardPulse(clip, paths.MotionTarget, 1.18f, MetersToLocal(0.20f, sceneScale.z));
            return clip;
        }
        ...
        private static AnimationClip BuildHitClip(AnimationPaths paths, Vector3 sceneScale)
        {
            ...
            SetVectorCurve(clip, paths.Model, "localPosition", 0f, Vector3.zero, 0.18f, MetersToLocal(new Vector3(-0.08f, -0.01f, -0.025f), sceneScale), 0.48f, MetersToLocal(new Vector3(-0.03f, 0.005f, -0.01f), sceneScale), 1.15f, Vector3.zero);
            SetVectorCurve(clip, paths.Model, "localScale", 0f, Vector3.one, 0.14f, new Vector3(1.10f, 0.84f, 1.06f), 0.42f, new Vector3(0.96f, 1.04f, 0.98f), 1.15f, Vector3.one);
            SetEulerZ(clip, paths.Model, 0f, 0f, 0.18f, 8f, 0.52f, -3f, 1.15f, 0f);
            SetWingEuler(clip, paths, 0f, 0f, 0.14f, -31f, 0.52f, -12f, 1.15f, 0f);
            AddMotionTargetHold(clip, paths.MotionTarget);
            return clip;
        }
        ...
        {
            var clip = CreateClip("Fuga_Death_FallAndFold", 1.85f);
            SetVectorCurve(clip, paths.Model, "localPosition", 0f, Vector3.zero, 0.55f, MetersToLocal(new Vector3(0f, -0.18f, -0.01f), sceneScale), 1.25f, MetersToLocal(new Vector3(0f, -0.56f, -0.02f), sceneScale), 1.85f, MetersToLocal(new Vector3(0f, -0.72f, -0.02f), sceneScale));
            SetVectorCurve(clip, paths.Model, "localScale", 0f, Vector3.one, 0.45f, new Vector3(1.02f, 0.96f, 1.02f), 1.85f, new Vector3(1.04f, 0.88f, 1.05f));
            SetEulerX(clip, paths.Model, 0f, 0f, 0.65f, 12f, 1.85f, 18f);
            SetWingEuler(clip, paths, 0f, 0f, 0.45f, -26f, 1.15f, -48f, 1.85f, -58f);
            AddMotionTargetHold(clip, paths.MotionTarget);
            return clip;
        }
        ...
        {
            var clip = CreateClip("Fuga_Consume_BiteForward", 1.1f);
            SetVectorCurve(clip, paths.Model, "localPosition", 0f, Vector3.zero, 0.32f, MetersToLocal(new Vector3(0f, 0.01f, 0.12f), sceneScale), 0.58f, MetersToLocal(new Vector3(0f, -0.015f, 0.16f), sceneScale), 1.1f, Vector3.zero);
            SetEulerX(clip, paths.Model, 0f, 0f, 0.32f, 9f, 0.58f, 16f, 1.1f, 0f);
            SetWingEuler(clip, paths, 0f, 0f, 0.32f, 10f, 0.58f, 4f, 1.1f, 0f);

            if (!string.IsNullOrEmpty(paths.LowerJaw))
            {
                SetEulerX(clip, paths.LowerJaw, 0f, 0f, 0.28f, 13f, 0.58f, 24f, 1.1f, 0f);
            }

            AddMotionTargetForwardPulse(clip, paths.MotionTarget, 1.1f, 0.42f);
            return clip;
        }
