...
{
    [InitializeOnLoad]
    internal static class NegatifClawAttackPlayModeCapture
    {
        ...
        private const string AnimatorStateName = "ClawAttack";
        private const double ClipDurationSeconds = 3.9d;
        private const string SessionStateKey =
            "Bellerophon.NegatifClawAttackPlayModeCapture.State";
        private const string SessionIndexKey =
            "Bellerophon.NegatifClawAttackPlayModeCapture.Index";
        private const string SessionStartTimeKey =
            "Bellerophon.NegatifClawAttackPlayModeCapture.StartTime";
        private const string SessionFailureKey =
            "Bellerophon.NegatifClawAttackPlayModeCapture.Failure";
        ...
        static NegatifClawAttackPlayModeCapture()
        {
            ...
            Action<Exception> failCallback)
        {
            ...
            if (EditorApplication.isPlayingOrWillChangePlaymode)
            {
                throw new InvalidOperationException(
                    "Cannot start the Negatif claw attack review while Unity is entering Play Mode.");
            }
            ...
            {
                throw new InvalidOperationException(
                    "CargoRunMvp must be clean before the Negatif claw attack review.");
            }
        ...
        private static void Tick()
        {
            ...
            try
            {
                ...
                if (state == Capturing)
                {
                    ...
                    {
                        throw new InvalidOperationException(
                            "Unity left Play Mode before the Negatif claw attack review finished.");
                    }
        ...
        private static void BeginRuntimePlayback()
        {
            ...
            {
                throw new InvalidOperationException(
                    "Negatif_03_Claw_Attack runtime Animator is not configured.");
            }
        ...
        private static void CaptureRuntimeFramesWhenDue()
        {
            ...
            {
                throw new InvalidOperationException(
                    "Negatif_03_Claw_Attack runtime Animator is missing.");
            }
            ...
            NegatifClawAttackAnimationTool.CaptureRuntimeFrame(PanelPath(index));
            index++;
            ...
            NegatifClawAttackAnimationTool.ComposeRuntimeReview(
                panelPaths,
                FinalReviewPath);
        ...
        private static Transform RequireSlot()
        {
            ...
            foreach (var root in scene.GetRootGameObjects())
            {
                ...
                return root.transform.Find(AttackSlotName) ??
                       throw new InvalidOperationException(
        ...
        private static void FinishSuccess()
        {
            ...
            CleanupSession();
            callback?.Invoke(
                "Negatif actual Play Mode claw attack review captured at 24 exact clip times from 0.05 through 3.9 seconds.");
        }
        ...
        private static void FinishFailure()
        {
            var error = SessionState.GetString(
                SessionFailureKey,
                "Negatif claw attack Play Mode capture failed.");
            var callback = fail;
            CleanupSession();
        ...
        private static string PanelPath(int index)
        {
            return Path.Combine(
                ProjectRoot,
                "Logs",
                "Negatif_ClawAttack_PlayMode_" + index + ".png");
        }
        ...
        private static string FinalReviewPath =>
            Path.Combine(
                ProjectRoot,
                "Logs",
                "Negatif_ClawAttack_VisualReview.png");
