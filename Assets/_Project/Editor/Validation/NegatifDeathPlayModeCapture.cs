...
{
    [InitializeOnLoad]
    internal static class NegatifDeathPlayModeCapture
    {
        ...
        private const string AnimatorStateName = "DeathRightRoll";
        private const double ClipDurationSeconds = 2.5d;
        private const string SessionStateKey =
            "Bellerophon.NegatifDeathPlayModeCapture.State";
        private const string SessionIndexKey =
            "Bellerophon.NegatifDeathPlayModeCapture.Index";
        private const string SessionStartTimeKey =
            "Bellerophon.NegatifDeathPlayModeCapture.StartTime";
        private const string SessionFailureKey =
            "Bellerophon.NegatifDeathPlayModeCapture.Failure";
        ...
        static NegatifDeathPlayModeCapture()
        {
            ...
            Action<Exception> failCallback)
        {
            ...
            if (EditorApplication.isPlayingOrWillChangePlaymode)
            {
                throw new InvalidOperationException(
                    "Cannot start the Negatif death review while Unity is entering Play Mode.");
            }
            ...
            {
                throw new InvalidOperationException(
                    "CargoRunMvp must be clean before the Negatif death review.");
            }
        ...
        private static void Tick()
        {
            ...
            try
            {
                ...
                if (state == Capturing)
                {
                    ...
                    {
                        throw new InvalidOperationException(
                            "Unity left Play Mode before the Negatif death review finished.");
                    }
        ...
        private static void BeginRuntimePlayback()
        {
            ...
            {
                throw new InvalidOperationException(
                    "Negatif_06_Death runtime Animator is not configured.");
            }
        ...
        private static void CaptureRuntimeFramesWhenDue()
        {
            ...
            {
                throw new InvalidOperationException(
                    "Negatif_06_Death runtime Animator is missing.");
            }
            ...
            NegatifDeathAnimationTool.CaptureRuntimeFrame(PanelPath(index));
            index++;
            ...
            NegatifDeathAnimationTool.ComposeRuntimeReview(
                panelPaths,
                FinalReviewPath);
        ...
        private static Transform RequireSlot()
        {
            ...
            foreach (var root in scene.GetRootGameObjects())
            {
                ...
                return root.transform.Find(DeathSlotName) ??
                       throw new InvalidOperationException(
        ...
        private static void FinishSuccess()
        {
            ...
            CleanupSession();
            callback?.Invoke(
                "Negatif actual Play Mode death review captured at 8 exact clip times from 0 through 2.45 seconds.");
        }
        ...
        private static void FinishFailure()
        {
            var error = SessionState.GetString(
                SessionFailureKey,
                "Negatif death Play Mode capture failed.");
            var callback = fail;
            CleanupSession();
        ...
        private static string PanelPath(int index)
        {
            return Path.Combine(
                ProjectRoot,
                "Logs",
                "Negatif_Death_PlayMode_" + index + ".png");
        }
        ...
        private static string FinalReviewPath =>
            Path.Combine(
                ProjectRoot,
                "Logs",
                "Negatif_Death_VisualReview.png");
