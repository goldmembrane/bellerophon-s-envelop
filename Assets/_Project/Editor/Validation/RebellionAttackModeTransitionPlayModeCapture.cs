...
{
    [InitializeOnLoad]
    internal static class RebellionAttackModeTransitionPlayModeCapture
    {
        ...
        private const string SessionStateKey =
            "Bellerophon.RebellionAttackTransitionCapture.State";
        private const string SessionIndexKey =
            "Bellerophon.RebellionAttackTransitionCapture.Index";
        private const string SessionStartTimeKey =
            "Bellerophon.RebellionAttackTransitionCapture.StartTime";
        private const string SessionFailureKey =
            "Bellerophon.RebellionAttackTransitionCapture.Failure";
        ...
        static RebellionAttackModeTransitionPlayModeCapture()
        {
            ...
            Action<Exception> failCallback)
        {
            ...
            if (EditorApplication.isPlayingOrWillChangePlaymode)
            {
                throw new InvalidOperationException(
                    "Cannot start the Rebellion attack transition review " +
                    "while Unity is entering Play Mode.");
            }
            ...
            {
                throw new InvalidOperationException(
                    "CargoRunMvp must be clean before the Rebellion attack " +
                    "transition review.");
            }
        ...
        private static void Tick()
        {
            ...
            try
            {
                ...
                if (state == Capturing)
                {
                    ...
                    {
                        throw new InvalidOperationException(
                            "Unity left Play Mode before the Rebellion attack " +
                            "transition review finished.");
                    }
        ...
        private static void BeginRuntimePlayback()
        {
            ...
            animator.Play(
                RebellionAttackModeTransitionTool.AnimatorStateName,
                0,
                0f);
            animator.Update(0f);
        ...
        private static void CaptureRuntimeFramesWhenDue()
        {
            ...
            var normalizedTime =
                (float)(CaptureTimes[index] /
                        RebellionAttackModeTransitionTool.LoopSeconds);
            animator.Play(
                RebellionAttackModeTransitionTool.AnimatorStateName,
                0,
                normalizedTime);
            animator.Update(0f);
            RebellionAttackModeTransitionTool.CaptureRuntimeFrame(
                PanelPath(index));
            index++;
            ...
            RebellionAttackModeTransitionTool.ComposeRuntimeReview(
                panels,
                RebellionAttackModeTransitionTool.FinalReviewAbsolutePath);
        ...
        private static void FinishSuccess()
        {
            ...
            CleanupSession();
            callback?.Invoke(
                "Rebellion actual Animator attack transition frames captured " +
                "across the complete 2.4-second loop.");
        }
        ...
        private static void FinishFailure()
        {
            var error = SessionState.GetString(
                SessionFailureKey,
                "Rebellion attack transition Play Mode capture failed.");
            var callback = fail;
            CleanupSession();
        ...
        private static string PanelPath(int index)
        {
            return Path.Combine(
                ProjectRoot,
                "Logs",
                "Rebellion_AttackTransition_PlayMode_" + index + ".png");
        }
