...
namespace Bellerophon.Editor.Validation
{
    public static class ApprovedCockpitDirectionBootstrap
    {
        ...
        private static readonly Vector3 LeftEngineTargetCenter = CockpitCenter + new Vector3(3.358f, 1.806f, 1.07278f);
        private static readonly Vector3 RightControlTargetCenter = CockpitCenter + new Vector3(-2.954f, 1.78f, 1.06727f);
        private static readonly Vector3 RearCargoTargetCenter = CockpitCenter + new Vector3(-1.66025f, 1.762f, 2.282f);
        ...
        private static readonly Quaternion RearCargoTargetRotation = Quaternion.Euler(-90f, 90.00001f, 0f);
        private static readonly string[] RemovedAfterEditorReviewObjectNameNeedles =
        {
            "ceiling route",
            "left wall",
            "right wall"
        ...
        public static void EnsureApprovedCockpitDirection()
        {
            ...
            var materials = LoadDirectionMaterials();
        ...
        private static void ValidateDirectionRootOnly()
        {
        ...
        public static void CaptureCurrentEditorObjects()
        {
            ...
            AppendOptionalGroupCandidate(builder, root.transform, "CeilingRouteTargetCenter", IsCeilingRoutePart, "ceiling route label");
        ...
        private static DirectionMaterials LoadDirectionMaterials()
        {
            return new DirectionMaterials(
                RequireMaterial(FrameMaterialPath),
                RequireMaterial(MountMaterialPath),
                RequireMaterial(EngineMaterialPath),
                RequireMaterial(ControlMaterialPath),
                RequireMaterial(CargoMaterialPath),
                RequireMaterial(CargoDimMaterialPath),
                RequireMaterial(TextMaterialPath),
                RequireMaterial(WearMaterialPath));
        }
        ...
        private static Material RequireMaterial(string path)
        {
            ...
            if (material == null)
            {
                throw new InvalidOperationException("Missing approved cockpit direction material: " + path);
            }
        ...
        private static void PositionApprovedGroups(Transform root)
        {
            ...
            MoveOptionalRendererGroup(root, IsLeftWallStripePart, LeftWallStripeTargetCenter);
            MoveOptionalRendererGroup(root, IsRightWallStripePart, RightWallStripeTargetCenter);
        }
        ...
        private static void RemoveEditorReviewObjects(Transform root)
        {
            ...
            for (var i = transforms.Length - 1; i >= 0; i--)
            {
                ...
                var lowerName = target.name.ToLowerInvariant();
                for (var j = 0; j < RemovedAfterEditorReviewObjectNameNeedles.Length; j++)
                {
                    if (lowerName.Contains(RemovedAfterEditorReviewObjectNameNeedles[j]))
                    {
                        UnityEngine.Object.DestroyImmediate(target.gameObject);
                        break;
                    }
        ...
        private static void ValidateEditorReviewRemovedObjects(Transform root)
        {
            ...
            for (var i = 0; i < transforms.Length; i++)
            {
                ...
                for (var j = 0; j < RemovedAfterEditorReviewObjectNameNeedles.Length; j++)
                {
                    if (lowerName.Contains(RemovedAfterEditorReviewObjectNameNeedles[j]))
                    {
                        throw new InvalidOperationException("Approved cockpit direction contains an editor-removed object: " + transforms[i].name);
                    }
                }
            }
        }
        ...
        private static void MoveOptionalRendererGroup(Transform root, Predicate<string> predicate, Vector3 targetCenter)
        {
            if (!TryGetGroupBounds(root, predicate, out var bounds))
            {
                return;
