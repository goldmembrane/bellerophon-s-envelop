...
{
    [InitializeOnLoad]
    internal static class NegatifHitReactionPlayModeCapture
    {
        ...
        private const string AnimatorStateName = "HitReaction";
        private const double ClipDurationSeconds = 0.7d;
        private const string SessionStateKey =
            "Bellerophon.NegatifHitReactionPlayModeCapture.State";
        private const string SessionIndexKey =
            "Bellerophon.NegatifHitReactionPlayModeCapture.Index";
        private const string SessionStartTimeKey =
            "Bellerophon.NegatifHitReactionPlayModeCapture.StartTime";
        private const string SessionFailureKey =
            "Bellerophon.NegatifHitReactionPlayModeCapture.Failure";
        ...
        static NegatifHitReactionPlayModeCapture()
        {
            ...
            Action<Exception> failCallback)
        {
            ...
            if (EditorApplication.isPlayingOrWillChangePlaymode)
            {
                throw new InvalidOperationException(
                    "Cannot start the Negatif hit reaction review while Unity is entering Play Mode.");
            }
            ...
            {
                throw new InvalidOperationException(
                    "CargoRunMvp must be clean before the Negatif hit reaction review.");
            }
        ...
        private static void Tick()
        {
            ...
            try
            {
                ...
                if (state == Capturing)
                {
                    ...
                    {
                        throw new InvalidOperationException(
                            "Unity left Play Mode before the Negatif hit reaction review finished.");
                    }
        ...
        private static void BeginRuntimePlayback()
        {
            ...
            {
                throw new InvalidOperationException(
                    "Negatif_04_Hit_Reaction runtime Animator is not configured.");
            }
        ...
        private static void CaptureRuntimeFramesWhenDue()
        {
            ...
            {
                throw new InvalidOperationException(
                    "Negatif_04_Hit_Reaction runtime Animator is missing.");
            }
            ...
            NegatifHitReactionAnimationTool.CaptureRuntimeFrame(PanelPath(index));
            index++;
            ...
            NegatifHitReactionAnimationTool.ComposeRuntimeReview(
                panelPaths,
                FinalReviewPath);
        ...
        private static Transform RequireSlot()
        {
            ...
            foreach (var root in scene.GetRootGameObjects())
            {
                ...
                return root.transform.Find(HitSlotName) ??
                       throw new InvalidOperationException(
        ...
        private static void FinishSuccess()
        {
            ...
            CleanupSession();
            callback?.Invoke(
                "Negatif actual Play Mode hit reaction review captured at 7 exact clip times from 0 through 0.7 seconds.");
        }
        ...
        private static void FinishFailure()
        {
            var error = SessionState.GetString(
                SessionFailureKey,
                "Negatif hit reaction Play Mode capture failed.");
            var callback = fail;
            CleanupSession();
        ...
        private static string PanelPath(int index)
        {
            return Path.Combine(
                ProjectRoot,
                "Logs",
                "Negatif_HitReaction_PlayMode_" + index + ".png");
        }
        ...
        private static string FinalReviewPath =>
            Path.Combine(
                ProjectRoot,
                "Logs",
                "Negatif_HitReaction_VisualReview.png");
