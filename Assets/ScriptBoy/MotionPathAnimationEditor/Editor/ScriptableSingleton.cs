    ...
    class ScriptableSingleton<T> : ScriptableObject where T : ScriptableObject
    {
        ...
        private static T FindInstance(string defaultPath)
        {
            ...
            if (obj == null)
            {
                ...
                if (guids.Length > 1)
                {
                    Debug.LogWarning($"MotionPathAnimEditor: There are multiple instances of the {typeof(T)}.");
                }

                if (obj == null) Debug.LogError($"MotionPathAnimEditor: Can't find the file in the following path \n {defaultPath}");
            }
        ...
        private new static T CreateInstance(string path)
        {
            ...
            Debug.Log($"MotionPathAnimEditor: The {typeof(T).Name} file just created.");
