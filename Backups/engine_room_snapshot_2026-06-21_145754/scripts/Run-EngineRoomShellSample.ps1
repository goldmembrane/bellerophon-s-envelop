...
$logPath = Join-Path $logDir "EngineRoomShellSample.log"
$stdoutPath = Join-Path $logDir "EngineRoomShellSample.stdout.log"
$stderrPath = Join-Path $logDir "EngineRoomShellSample.stderr.log"
$scriptPath = Join-Path $projectRoot "scripts\GenerateEngineRoomShellSample.py"
$sampleRoot = Join-Path $projectRoot "artSample\engine_room_shell"
...
foreach ($relative in $requiredFiles) {
  ...
  if (-not (Test-Path -LiteralPath $path)) {
    Write-Error "Missing generated engine room shell sample file: $path"
    exit 1
  }
}
