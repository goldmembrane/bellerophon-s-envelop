...
namespace Bellerophon.Editor.LongaArmaCargoRunScene
{
    internal static class LongaArmaCargoRunSceneApplyAndReview
    {
        ...
        private const string SourceModelRelativePath = RuntimeLowPolySampleRootRelativePath + "/exports/longa_arma_runtime_lowpoly_rigged.fbx";
        ...
        private const string MoveClipName = "LongaArma_Move_Crawl";
        private const string AttackClipName = "LongaArma_Attack_SlamDrag";
        private const string HitClipName = "LongaArma_Hit_Recoil";
        private const string ConsumeClipName = "LongaArma_Consume_Peck";
        private const string DeathClipName = "LongaArma_Death_MeltPuddle";
        ...
        private const string RootBoneName = "DEF_root";
        private const string SpineBoneName = "DEF_spine_01";
        private const string ChestBoneName = "DEF_chest";
        private const string HeadBoneName = "DEF_head";
        private const string BladeArmBoneName = "DEF_blade_upper_l";
        private const string BladeArmForearmBoneName = "DEF_blade_fore_l";
        private const string BladeArmTipBoneName = "DEF_blade_tip_l";
        private const string FrontRightLegBoneName = "DEF_front_right_upper";
        private const string FrontRightLowerLegBoneName = "DEF_front_right_lower";
        private const string FrontRightFootBoneName = "DEF_front_right_foot";
        private const string FrontLeftLegBoneName = "DEF_front_left_upper";
        private const string FrontLeftLowerLegBoneName = "DEF_front_left_lower";
        private const string FrontLeftFootBoneName = "DEF_front_left_foot";
        private const string RearRightLegBoneName = "DEF_rear_right_upper";
        private const string RearRightLowerLegBoneName = "DEF_rear_right_lower";
        private const string RearRightFootBoneName = "DEF_rear_right_foot";
        private const string RearLeftLegBoneName = "DEF_rear_left_upper";
        private const string RearLeftLowerLegBoneName = "DEF_rear_left_lower";
        private const string RearLeftFootBoneName = "DEF_rear_left_foot";
        ...
        private static readonly AnimationStateSpec[] AnimationStateSpecs =
        {
            ...
            new("LongaArma_02_Move_Crawl", MoveClipName, new Vector3(-2.10f, 0f, 0f), 0.55f, true),
            new("LongaArma_03_Attack_SlamDrag", AttackClipName, Vector3.zero, 1.08f, true),
            new("LongaArma_04_Hit_Recoil", HitClipName, new Vector3(2.10f, 0f, 0f), 0.12f, true),
            new("LongaArma_05_Consume_Peck", ConsumeClipName, new Vector3(4.20f, 0f, 0f), 0.58f, true),
            new("LongaArma_06_Death_MeltPuddle", DeathClipName, new Vector3(6.30f, 0f, 0f), 1.65f, true)
        };
        ...
        {
            var modelRoot = RequireAnimationModelRoot(prefab);
            var skinnedRenderer = RequireLongaSkinnedRenderer(prefab);
            ValidateRequiredBlendShapes(skinnedRenderer, prefab.name);
            RequireRuntimeBoneBindings(prefab);

            var rendererPath = AnimationUtility.CalculateTransformPath(skinnedRenderer.transform, modelRoot);
            ...
            {
                if (clips.ContainsKey(spec.ClipName))
                {
                    continue;
                }

                var importedClip = FindImportedAnimationClip(importedClips, spec.ClipName);
                AddSavedImportedAnimationClip(clips, importedClip, spec.ClipName, spec.Loop, rendererPath);
            }
        ...
        private static Transform RequireAnimationModelRoot(GameObject root)
        {
            var animationPivot = root.transform.Find(AnimationPivotChildName);
            ...
            var modelRoot = animationPivot.Find(ModelChildName);
            if (modelRoot == null)
            {
                throw new InvalidOperationException($"Longa Arma prefab must contain {AnimationPivotChildName}/{ModelChildName} for imported Blender Action playback.");
            }

            return modelRoot;
        }

        private static List<AnimationClip> LoadImportedAnimationClips()
        {
            var importedClips = new List<AnimationClip>();
            foreach (var asset in AssetDatabase.LoadAllAssetRepresentationsAtPath(UnityModelAssetPath))
            {
            ...
            {
                throw new InvalidOperationException($"No Blender Action animation clips were imported from {UnityModelAssetPath}.");
            }

            return importedClips;
        }

        private static AnimationClip FindImportedAnimationClip(List<AnimationClip> importedClips, string clipName)
        {
            ...
            foreach (var clip in importedClips)
            {
                if (clip.name.EndsWith("|" + clipName, StringComparison.Ordinal) ||
                    clip.name.Contains(clipName, StringComparison.Ordinal))
                {
                    if (fallback != null)
                    {
                        throw new InvalidOperationException($"Multiple imported Longa Arma clips match {clipName}: {fallback.name}, {clip.name}.");
                    }
            ...
            throw new InvalidOperationException($"Imported Longa Arma clip is missing: {clipName}. Available clips: {BuildImportedClipNameList(importedClips)}");
        }

        private static string BuildImportedClipNameList(List<AnimationClip> importedClips)
        {
        ...
        private static void AddSavedImportedAnimationClip(
            ...
            string rendererPath)
        {
            var clipPath = AnimationRootPath + "/" + clipName + ".anim";
            if (AssetDatabase.LoadAssetAtPath<AnimationClip>(clipPath) != null)
            {
                AssetDatabase.DeleteAsset(clipPath);
            }

            var editableClip = UnityEngine.Object.Instantiate(importedClip);
            editableClip.name = clipName;
            ConfigureLoopSetting(editableClip, loop);
            RemoveStateSpecificImportedBlendShapeCurves(editableClip, clipName);
            ApplySupplementalMorphCurves(editableClip, clipName, rendererPath);

            AssetDatabase.CreateAsset(editableClip, clipPath);
            var savedClip = AssetDatabase.LoadAssetAtPath<AnimationClip>(clipPath);
            if (savedClip == null)
            {
                throw new InvalidOperationException($"Could not create imported Longa Arma animation clip at {clipPath}.");
            }

            ConfigureLoopSetting(savedClip, loop);
            ValidateImportedClipBindings(savedClip, clipName);
            EditorUtility.SetDirty(savedClip);
            clips[clipName] = savedClip;
        }

        private static void RemoveStateSpecificImportedBlendShapeCurves(AnimationClip clip, string clipName)
        {
            if (clipName == MoveClipName || clipName == AttackClipName)
            {
                RemoveBlendShapeCurvesWithPrefix(clip, "blendShape.");
            }
        }

        private static void RemoveBlendShapeCurvesWithPrefix(AnimationClip clip, string propertyPrefix)
        {
            foreach (var binding in AnimationUtility.GetCurveBindings(clip))
            {
                if (binding.propertyName.StartsWith(propertyPrefix, StringComparison.Ordinal))
                {
        ...
        private static void ApplySupplementalMorphCurves(AnimationClip clip, string clipName, string rendererPath)
        {
            switch (clipName)
            {
                case IdleClipName:
        ...
        private static void ValidateImportedClipBindings(AnimationClip clip, string clipName)
        {
            var curveBindings = AnimationUtility.GetCurveBindings(clip);
            if (curveBindings.Length == 0)
            {
                throw new InvalidOperationException($"{clipName} has no imported Blender Action animation curves.");
            }

            if (clipName == MoveClipName)
            {
                RequireClipBindingPathContains(clip, clipName, FrontRightFootBoneName);
                RequireClipBindingPathContains(clip, clipName, RearLeftFootBoneName);
                RejectClipBlendShapePrefix(clip, clipName, "blendShape.Move_");
            }
            else if (clipName == AttackClipName)
            {
                RequireClipBindingPathContains(clip, clipName, BladeArmTipBoneName);
                RequireClipBindingPathContains(clip, clipName, FrontRightFootBoneName);
                RejectClipBlendShapePrefix(clip, clipName, "blendShape.Attack_");
            }
            else if (clipName == IdleClipName)
            {
                RequireClipProperty(clip, clipName, "blendShape." + IdleShapeKeyName);
            }
            else if (clipName == DeathClipName)
            {
                RequireClipProperty(clip, clipName, "blendShape." + DeathPuddleShapeKeyName);
            }
        }

        private static void RequireClipBindingPathContains(AnimationClip clip, string clipName, string pathPart)
        {
            foreach (var binding in AnimationUtility.GetCurveBindings(clip))
            {
                if (binding.path.Contains(pathPart, StringComparison.Ordinal))
                {
                    return;
                }
            }

            throw new InvalidOperationException($"{clipName} does not contain a Blender Action curve for {pathPart}.");
        }

        private static void RequireClipProperty(AnimationClip clip, string clipName, string propertyName)
        {
            foreach (var binding in AnimationUtility.GetCurveBindings(clip))
            {
            ...
            throw new InvalidOperationException($"{clipName} does not contain required curve {propertyName}.");
        }

        private static void RejectClipBlendShapePrefix(AnimationClip clip, string clipName, string propertyPrefix)
        {
            foreach (var binding in AnimationUtility.GetCurveBindings(clip))
            {
                if (binding.propertyName.StartsWith(propertyPrefix, StringComparison.Ordinal))
                {
                    throw new InvalidOperationException($"{clipName} must use Blender bone animation for this state, but contains {binding.propertyName}.");
                }
            }
        }
            ...
            Dictionary<string, AnimatorController> controllers)
        {
            ...
            var rootAnimator = instance.GetComponent<Animator>();
            if (rootAnimator != null)
            {
                UnityEngine.Object.DestroyImmediate(rootAnimator);
            }

            var modelRoot = RequireAnimationModelRoot(instance);
            ...
            clip.SampleAnimation(modelRoot.gameObject, Mathf.Clamp(spec.SampleTime, 0f, Mathf.Max(clip.length, spec.SampleTime)));
        ...
        private static void InspectSceneState(Transform placementRoot)
        {
            foreach (var spec in AnimationStateSpecs)
            {
                ...
                var modelRoot = RequireAnimationModelRoot(child.gameObject);
                var animator = modelRoot.GetComponent<Animator>();
                if (animator == null || animator.runtimeAnimatorController == null)
                {
                    throw new InvalidOperationException($"{spec.ObjectName} must have an AnimatorController assigned on {AnimationPivotChildName}/{ModelChildName}.");
                }
