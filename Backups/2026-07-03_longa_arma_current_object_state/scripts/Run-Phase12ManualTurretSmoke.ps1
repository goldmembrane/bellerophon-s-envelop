...
$requestPath = Join-Path $projectRoot "Logs\Phase12ManualTurretSmoke.request"
$activePath = Join-Path $projectRoot "Logs\Phase12ManualTurretSmoke.active"
$logPath = Join-Path $projectRoot "Logs\Phase12ManualTurretSmoke.log"
$timeoutSeconds = 60
...
if (Test-Path -LiteralPath $activePath) {
  ...
  if ($activeItem.LastWriteTime -lt (Get-Date).AddMinutes(-2)) {
    ...
    Write-Error "A Phase 12 manual turret smoke request is already active. See $activePath"
    exit 1
  }
}

$requestId = [guid]::NewGuid().ToString("N")
Set-Content -LiteralPath $logPath -Value "Phase 12 manual turret smoke pending: $requestId" -Encoding UTF8
@(
  "id=$requestId",
  "logPath=$logPath"
...
while ((Get-Date) -lt $deadline) {
  if (Test-Path -LiteralPath $logPath) {
    ...
    if ($log -match "Phase 12 manual turret smoke completed: $requestId") {
      if ($log -match "Result: Passed") {
        exit 0
      }

      Write-Error "Phase 12 manual turret smoke failed. See $logPath"
      exit 1
    }
  }
...
Write-Error "Phase 12 manual turret smoke timed out after $timeoutSeconds seconds. See $logPath"
exit 1
