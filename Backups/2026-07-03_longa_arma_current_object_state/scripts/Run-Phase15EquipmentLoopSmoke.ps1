...
$requestPath = Join-Path $projectRoot "Logs\Phase15EquipmentLoopSmoke.request"
$activePath = Join-Path $projectRoot "Logs\Phase15EquipmentLoopSmoke.active"
$logPath = Join-Path $projectRoot "Logs\Phase15EquipmentLoopSmoke.log"
$timeoutSeconds = 70
...
if (Test-Path -LiteralPath $activePath) {
  ...
  if ($activeItem.LastWriteTime -lt (Get-Date).AddMinutes(-2)) {
    ...
    Write-Error "A Phase 15 equipment loop smoke request is already active. See $activePath"
    exit 1
  }
}

$requestId = [guid]::NewGuid().ToString("N")
Set-Content -LiteralPath $logPath -Value "Phase 15 equipment loop smoke pending: $requestId" -Encoding UTF8
@(
  "id=$requestId",
  "logPath=$logPath"
...
while ((Get-Date) -lt $deadline) {
  if (Test-Path -LiteralPath $logPath) {
    ...
    if ($log -match "Phase 15 equipment loop smoke completed: $requestId") {
      if ($log -match "Result: Passed") {
        exit 0
      }

      Write-Error "Phase 15 equipment loop smoke failed. See $logPath"
      exit 1
    }
  }
...
Write-Error "Phase 15 equipment loop smoke timed out after $timeoutSeconds seconds. See $logPath"
exit 1
