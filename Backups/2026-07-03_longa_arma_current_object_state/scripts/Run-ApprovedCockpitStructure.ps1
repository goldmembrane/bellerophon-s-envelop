...
$applyLogPath = Join-Path $logDir "ApprovedCockpitStructure.log"
$captureLogPath = Join-Path $logDir "ApprovedCockpitStructureComparison.log"
...
& (Join-Path $PSScriptRoot "Invoke-UnityEditorBridge.ps1") -Command "EnsureApprovedCockpitStructure" -LogPath $applyLogPath -TimeoutSeconds 240
if ($LASTEXITCODE -ne 0) {
  exit $LASTEXITCODE
}

& (Join-Path $PSScriptRoot "Invoke-UnityEditorBridge.ps1") -Command "CaptureApprovedCockpitStructureComparison" -LogPath $captureLogPath -TimeoutSeconds 240
if ($LASTEXITCODE -ne 0) {
  exit $LASTEXITCODE
}
...
if ($applyLog -notmatch "Approved cockpit 01 structure applied\.") {
  Write-Error "Approved cockpit apply success marker was not found. See $applyLogPath"
  exit 1
}

if ($applyLog -notmatch "Approved cockpit 01 structure validation passed\.") {
  Write-Error "Approved cockpit validation marker was not found. See $applyLogPath"
  exit 1
}
...
if ($captureLog -notmatch "Approved cockpit 01 Unity comparison snapshots saved:") {
  Write-Error "Approved cockpit comparison capture marker was not found. See $captureLogPath"
  exit 1
}
...
if ($applyLog -match "Scripts have compiler errors|error CS\d+|Unity editor bridge failed" -or
  ...
  Write-Error "Approved cockpit structure logs contain errors. See $applyLogPath and $captureLogPath"
  exit 1
}
