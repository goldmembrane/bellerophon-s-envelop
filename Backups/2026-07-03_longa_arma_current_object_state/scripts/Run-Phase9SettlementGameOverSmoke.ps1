...
$requestPath = Join-Path $projectRoot "Logs\Phase9SettlementGameOverSmoke.request"
$activePath = Join-Path $projectRoot "Logs\Phase9SettlementGameOverSmoke.active"
$logPath = Join-Path $projectRoot "Logs\Phase9SettlementGameOverSmoke.log"
$timeoutSeconds = 60
...
if (Test-Path -LiteralPath $activePath) {
  ...
  if ($activeItem.LastWriteTime -lt (Get-Date).AddMinutes(-2)) {
    ...
    Write-Error "A Phase 9 settlement game over smoke request is already active. See $activePath"
    exit 1
  }
}

$requestId = [guid]::NewGuid().ToString("N")
Set-Content -LiteralPath $logPath -Value "Phase 9 settlement game over smoke pending: $requestId" -Encoding UTF8
@(
  "id=$requestId",
  "logPath=$logPath"
...
while ((Get-Date) -lt $deadline) {
  if (Test-Path -LiteralPath $logPath) {
    ...
    if ($log -match "Phase 9 settlement game over smoke completed: $requestId") {
      if ($log -match "Result: Passed") {
        exit 0
      }

      Write-Error "Phase 9 settlement game over smoke failed. See $logPath"
      exit 1
    }
  }
...
Write-Error "Phase 9 settlement game over smoke timed out after $timeoutSeconds seconds. See $logPath"
exit 1
