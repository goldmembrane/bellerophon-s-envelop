...
$requestPath = Join-Path $projectRoot "Logs\Phase16HudMapAtmosphereSmoke.request"
$activePath = Join-Path $projectRoot "Logs\Phase16HudMapAtmosphereSmoke.active"
$logPath = Join-Path $projectRoot "Logs\Phase16HudMapAtmosphereSmoke.log"
$timeoutSeconds = 75
...
if (Test-Path -LiteralPath $activePath) {
  ...
  if ($activeItem.LastWriteTime -lt (Get-Date).AddMinutes(-2)) {
    ...
    Write-Error "A Phase 16 HUD map atmosphere smoke request is already active. See $activePath"
    exit 1
  }
}

$requestId = [guid]::NewGuid().ToString("N")
Set-Content -LiteralPath $logPath -Value "Phase 16 HUD map atmosphere smoke pending: $requestId" -Encoding UTF8
@(
  "id=$requestId",
  "logPath=$logPath"
...
while ((Get-Date) -lt $deadline) {
  if (Test-Path -LiteralPath $logPath) {
    ...
    if ($log -match "Phase 16 HUD map atmosphere smoke completed: $requestId") {
      if ($log -match "Result: Passed") {
        exit 0
      }

      Write-Error "Phase 16 HUD map atmosphere smoke failed. See $logPath"
      exit 1
    }
  }
...
Write-Error "Phase 16 HUD map atmosphere smoke timed out after $timeoutSeconds seconds. See $logPath"
exit 1
