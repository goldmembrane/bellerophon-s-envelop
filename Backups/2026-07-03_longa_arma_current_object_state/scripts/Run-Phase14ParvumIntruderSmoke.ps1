...
$requestPath = Join-Path $projectRoot "Logs\Phase14ParvumIntruderSmoke.request"
$activePath = Join-Path $projectRoot "Logs\Phase14ParvumIntruderSmoke.active"
$logPath = Join-Path $projectRoot "Logs\Phase14ParvumIntruderSmoke.log"
$timeoutSeconds = 60
...
if (Test-Path -LiteralPath $activePath) {
  ...
  if ($activeItem.LastWriteTime -lt (Get-Date).AddMinutes(-2)) {
    ...
    Write-Error "A Phase 14 parvum intruder smoke request is already active. See $activePath"
    exit 1
  }
}

$requestId = [guid]::NewGuid().ToString("N")
Set-Content -LiteralPath $logPath -Value "Phase 14 parvum intruder smoke pending: $requestId" -Encoding UTF8
@(
  "id=$requestId",
  "logPath=$logPath"
...
while ((Get-Date) -lt $deadline) {
  if (Test-Path -LiteralPath $logPath) {
    ...
    if ($log -match "Phase 14 parvum intruder smoke completed: $requestId") {
      if ($log -match "Result: Passed") {
        exit 0
      }

      Write-Error "Phase 14 parvum intruder smoke failed. See $logPath"
      exit 1
    }
  }
...
Write-Error "Phase 14 parvum intruder smoke timed out after $timeoutSeconds seconds. See $logPath"
exit 1
