...
$requestPath = Join-Path $projectRoot "Logs\Phase11AsteroidHazardSmoke.request"
$activePath = Join-Path $projectRoot "Logs\Phase11AsteroidHazardSmoke.active"
$logPath = Join-Path $projectRoot "Logs\Phase11AsteroidHazardSmoke.log"
$timeoutSeconds = 60
...
if (Test-Path -LiteralPath $activePath) {
  ...
  if ($activeItem.LastWriteTime -lt (Get-Date).AddMinutes(-2)) {
    ...
    Write-Error "A Phase 11 asteroid hazard smoke request is already active. See $activePath"
    exit 1
  }
}

$requestId = [guid]::NewGuid().ToString("N")
Set-Content -LiteralPath $logPath -Value "Phase 11 asteroid hazard smoke pending: $requestId" -Encoding UTF8
@(
  "id=$requestId",
  "logPath=$logPath"
...
while ((Get-Date) -lt $deadline) {
  if (Test-Path -LiteralPath $logPath) {
    ...
    if ($log -match "Phase 11 asteroid hazard smoke completed: $requestId") {
      if ($log -match "Result: Passed") {
        exit 0
      }

      Write-Error "Phase 11 asteroid hazard smoke failed. See $logPath"
      exit 1
    }
  }
...
Write-Error "Phase 11 asteroid hazard smoke timed out after $timeoutSeconds seconds. See $logPath"
exit 1
